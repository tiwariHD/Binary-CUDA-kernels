				...
				const int stride, float* data_col, float pad_val)
{
	...
	im2col_gpu_kernel<float><<<CAFFE_GET_BLOCKS(num_kernels),
		CAFFE_CUDA_NUM_THREADS>>>(
		num_kernels, data_im, height, width, ksize, pad, stride, height_col,
		width_col, data_col, pad_val);
				...
				const int stride, unsigned int* data_col, unsigned int pad_val)
{
	...
	im2col_gpu_kernel<unsigned int><<<CAFFE_GET_BLOCKS(num_kernels),
		CAFFE_CUDA_NUM_THREADS>>>(
		num_kernels, data_im, height, width, ksize, pad, stride, height_col,
		width_col, data_col, pad_val);
	CUDA_POST_KERNEL_CHECK;
...
__global__ void lifting(const float* data_in, float* data_out, int rowSize, int size)
{
